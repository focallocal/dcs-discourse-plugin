   ...
   */
	didTransition(route) {
		...
		if (url !== this.currentUrl) {
			this.clientContext = null // New page = new contextContext
			this._loadPage({
				url,
   ...
   */
	onSetRouteProps(args) {
		...
		if (discourseTitle) {
			...
			const titlePrefixes = document.querySelectorAll('.dcs-title-prefix')
			titlePrefixes.forEach(el => el.remove())
      ...
      if (navContainer) {
        ...
        navContainer.insertBefore(h1, navContainer.firstChild)
      }
      */
			const tagHeading = document.querySelector('.tag-show-heading')
			if (tagHeading) {
				tagHeading.textContent = safeTitle
				tagHeading.style.display = 'inline-flex'
			}

			// In topic route, we transform the topic title. The issue here is that
			// the title is rendered very late, so we need to wait until the title
			...
			const hasTitle = () => {
				...
				return title && title.textContent.length > 0 ? title : null
			}
			u.async
				...
				.then(
					title => {
						if (this.currentRoute.interactMode === 'COMMENT') {
							...
							// Is there a topic map? The topic map is the grey rectangular area
							...
							if (topicMap) {
								...
								if (topicTitleEl) {
									topicTitleEl.style.marginTop = '50px'
								}
							}
						} else {
